...
def test(args, model, tokenizer, prefix="",pool=None,best_threshold=0):
    ...
    y_trues = []
    num_incorrect = 0
    incorrect_indices = []  # 用于记录不一致的样本索引
    for i, batch in tqdm(enumerate(eval_dataloader)):
        ...
        with torch.no_grad():
            ...
            y_trues.append(labels.cpu().numpy())
            # 计算预测标签
            y_preds = logit.argmax(dim=1)
            # 检查预测标签与实际标签是否一致
            num_incorrect += torch.sum(y_preds != labels).item()
            # 如果不一致，记录索引
            if (y_preds != labels).any():
    ...
    result["num_incorrect"] = num_incorrect

    # 输出不一致的样本索引
    logger.info("***** Test results {} *****".format(prefix))
    logger.info("Number of incorrect predictions: %d", num_incorrect)
    logger.info("Incorrect sample indices: %s", incorrect_indices)
    for key in sorted(result.keys()):
        logger.info("  %s = %s", key, str(round(result[key], 4)))
    return result
